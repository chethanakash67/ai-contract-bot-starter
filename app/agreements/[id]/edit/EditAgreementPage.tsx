...
export default function EditAgreementPage({ params }: { params: { id: string } }) {
  ...
  const handleAction = async (action: 'generate' | 'save' | 'validate' | 'toPdf' | 'toDocx' | 'toTex' | 'share') => {
    ...
    const promise = (async () => {
      ...
      switch (action) {
        case 'generate': {
          const genRes = await fetch(`/api/agreements/${params.id}/generate`, { method: "POST" });
          if (!genRes.ok) {
            let msg = 'Failed to generate draft';
            try { const j = await genRes.json(); msg = j?.error || msg; } catch {}
            throw new Error(msg);
          }
          const refreshed = await fetch(`/api/agreements/${params.id}`, { cache: 'no-store' }).then(r => r.json());
          const newHtml = refreshed?.draftHtml || '';
          setHtml(newHtml);
          setAgreementData((prev) => ({ ...(refreshed || prev) }));
          editor?.commands.setContent(newHtml, false);
          break;
        }
        case 'save':
          // If not in draft, skip save to avoid read-only errors
          if ((agreementData?.status || 'draft') !== 'draft') {
            return { skipped: true } as any;
          }
          result = await fetch(`/api/agreements/${params.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ draftHtml: html })
          });
          if (!result.ok) {
            let msg = 'Failed to save';
            try { const j = await result.json(); msg = j?.error || msg; } catch {}
            throw new Error(msg);
          }
          break;
        case 'validate': {
    ...
    // Ensure any rejection from the original promise is handled to avoid unhandled runtime errors
    promise.catch(() => {});
  };
